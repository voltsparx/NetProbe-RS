...
use crate::platform::{privilege, self_integrity};
...
fn ensure_supported_host_platform() -> NProbeResult<()> {
    if cfg!(target_os = "linux") || cfg!(target_os = "macos") || cfg!(windows) {
        return Ok(());
    }

    Err(NProbeError::Config(
        "nprobe-rs currently supports Linux, macOS, and Windows hosts.".to_string(),
    ))
}
...
fn maybe_reexec_with_root(request: &ScanRequest, raw_args: &[OsString]) -> NProbeResult<()> {
    ...
    if !request.requires_root() || privilege::has_elevated_network_privileges() {
        return Ok(());
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    {
        if !has_interactive_tty() {
            return Err(NProbeError::Safety(
                "root privileges are required, but no interactive terminal is available for password prompt. Re-run with sudo or doas manually."
                    .to_string(),
            ));
        }
